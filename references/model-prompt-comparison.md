...
| **LoRA 支持** | ✅ 支持（使用 Flux LoRA） | ✅ 支持（使用 SDXL LoRA） | ✅ 支持（使用 Flux Turbo LoRA） |
| **大小写** | 全小写 | 不敏感 | 不敏感 |
...
5. **LoRA 支持** - 支持 `<lora:xxx:weight>` 格式。**注意**：LoRA 文件分模型架构（SDXL/Flux），SDXL 训练的 LoRA 不能在 Flux 上用，需使用对应 Flux 版本的 LoRA。引用格式完全相同。
...
- Flux/Anima LoRA 不能在 SDXL 上用，反之亦然。引用格式相同
- 必须使用 `<lora:...>` 括号包裹
