...
class FEEncLoraAutoLoaderStack(LoraLoader):
    ...
    def __init__(self):
        super().__init__()
        # self.BASE_LORADIR_CACHE = [os.path.basename(x).split('.')[0] for x in self.LORADIR_CACHE]
        self.BASE_LORADIR_CACHE = [os.path.basename(self.LORADIR_CACHE[x]).split('.')[0] for x in range(len(self.LORADIR_CACHE))]
    ...
    def load_lora_stack(self, model_type, prompt, strength_model, strength_clip):
        ...
        for str_lora_item in arr:
            ...
            # print(f"Auto Load lora {loar_sim_path}")
            # print(f"self.BASE_LORADIR_CACHE {self.BASE_LORADIR_CACHE}")
            # print(f"self.LORADIR_CACHE {self.LORADIR_CACHE}")
            lora_name = self.LORADIR_CACHE[self.BASE_LORADIR_CACHE.index(loar_sim_path)] if loar_sim_path in self.BASE_LORADIR_CACHE else None
            print(f"Auto Load lora {lora_name}")
